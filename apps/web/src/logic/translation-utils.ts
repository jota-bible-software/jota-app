import { TranslationContent, TranslationContentArray, TranslationContentMap } from 'src/types'
 ...
 */
export function getVerseCount(content: TranslationContent, bookIndex: number, chapterIndex: number): number {
  if (isArrayFormat(content)) {
...
export function toMapFormat(content: TranslationContent): TranslationContentMap {
  ...
  // At this point, content must be array format
  const arrayContent = content as TranslationContentArray
  const result: TranslationContentMap = {}

  arrayContent.forEach((book: string[][], bookIndex: number) => {
    const bookNumber = bookIndex + 1
    result[bookNumber] = {}

    book.forEach((chapter: string[], chapterIndex: number) => {
      const chapterNumber = chapterIndex + 1
