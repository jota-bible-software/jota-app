...
import { CopyTemplateData, FormatTemplateData } from 'src/types'
...
const names = computed(() => items.value.map((it: CopyTemplateData) => it.name))
const selected = ref('')
...
function formatItem(item: CopyTemplateData) {
  const formatTemplate = settings.persist.formatTemplates?.find((it: FormatTemplateData) => it.name === item.formatTemplate)
...
function add() {
  const theSame = items.value.find((it: CopyTemplateData) => it.formatTemplate === newItem.value.formatTemplate && it.bookNaming === newItem.value.bookNaming)
  if (theSame) {
    Dialog.create({
...
function save() {
  const index = items.value.findIndex((it: CopyTemplateData) => it.name === selected.value)
  if (index !== -1) {
...
function remove() {
  Dialog.create({
    ...
    const i = items.value.findIndex((it: CopyTemplateData) => it.name === selected.value)
    if (i !== -1) {
