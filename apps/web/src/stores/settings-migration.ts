...
export function migrateV5ToV6(persist: SettingsPersist) {
  ...
  // This creates the legacy format which will be migrated to new format in later migration
  if (!persist.highlights) {
    const legacyHighlights: HighlightsLegacy = {
    ...
    // Store as legacy format - will be migrated to new format later
    ;(persist as unknown as { highlights: HighlightsLegacy }).highlights = legacyHighlights
  }
}
