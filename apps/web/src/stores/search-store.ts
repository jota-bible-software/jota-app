...
export const useSearchStore = defineStore('search', () => {
  ...
  async function findByInput(input: string, options: SearchOptions) {
    ...
    const progressRunner: Progress = {
      step: (value) => {
        // Get book count for progress calculation - works with both array and map formats
        const content = translationContent.value
        const bookCount = content
          ? (Array.isArray(content) ? content.length : Object.keys(content).length)
          : 1
        progress.value = value / bookCount
      }
    }
